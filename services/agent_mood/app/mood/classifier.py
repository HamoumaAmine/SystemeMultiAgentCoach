...
# chemin : .../SystemeMultiAgentCoach/services/agent_mood/app/mood/classifier.py
...
class MoodResult:
    mood: str           # ex: "fatigue"
    score: float        # 0.0 à 1.0 (émotion négative -> 0, positive -> 1)
...
def _fallback_simple(text: str) -> MoodResult:
    ...
    if any(w in lower for w in ["epuise", "fatigue", "j en peux plus"]):
        return MoodResult(
            mood="fatigue",
            score=0.2,  # émotion négative forte -> proche de 0
            valence="negative",
            energy="low",
    ...
    if any(w in lower for w in ["motive", "en forme", "au top", "super bien"]):
        return MoodResult(
            mood="positif",
            score=0.8,  # émotion positive forte -> proche de 1
            valence="positive",
            energy="high",
